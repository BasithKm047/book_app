...
String adminBox='admin';

ValueNotifier<List<AdminModel>>adminListNotifier=ValueNotifier([]);
// Function to add an Admin
Future<void> addAdmin(AdminModel admin) async {
  try {
    final adminDb = await Hive.openBox<AdminModel>(adminBox);
    await adminDb.put(admin.id, admin);
    adminListNotifier.notifyListeners();
...
Future<List> getAdmindetails()async{
  final adminDb=await Hive.openBox<AdminModel>(adminBox);
  final adminList=adminDb.values.toList();
  adminListNotifier.value=adminList;
  adminListNotifier.notifyListeners();
...
// class HiveService {
//   static Box<AdminModel>? _adminBox;

//   static Future<Box<AdminModel>> getAdminBox() async {
//     if (_adminBox == null || !_adminBox!.isOpen) {
//       _adminBox = await Hive.openBox<AdminModel>('admin');
//     }
//     return _adminBox!;
//   }
// }
